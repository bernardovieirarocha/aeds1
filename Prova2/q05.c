// Faça um programa que leia do usuário dois valores n e m e, em seguida, leia
// uma matriz de caracteres (exemplo abaixo) n × m e imprima SIM se houver um
// caminho até a saída desse labirinto, ou NAO, se não houver. Sabe-se que
// sempre existe exatamente duas aberturas: uma no ponto (1, 0) e outra no
// ponto (0, m − 2). ATENÇÃO! A solução deve ser encontrada por meio de recursão e os únicos movimentos possíveis para um espaço em branco são: para cima; para baixo; para esquerda; e para direita.
